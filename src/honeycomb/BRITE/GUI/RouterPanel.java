...
public final class RouterPanel extends JPanel  implements ActionListener {
 ...
 /** converts import format to brite format and returns name of new file. */
  public void ConvertFileToBriteFormat() {
    ...
    honeycomb.BRITE.Util.Util.MSG("Converting Router-level import file to BRITE..");
    
    String briteFile=file+".brite";
    if (extFormat.equals("nlanr")) {
    	honeycomb.BRITE.Import.NLANRImport im = new honeycomb.BRITE.Import.NLANRImport(new File(convertFilename), honeycomb.BRITE.Model.ModelConstants.RT_FILE);
	im.convert(briteFile);
    }
    else   if (extFormat.equals("gtitm")) {
    	honeycomb.BRITE.Import.GTImport im = new honeycomb.BRITE.Import.GTImport(new File(convertFilename), honeycomb.BRITE.Model.ModelConstants.RT_FILE);
      im.convert(briteFile);
    }    
    else if (extFormat.equals("gtts")) {
    	honeycomb.BRITE.Import.GTTSImport im = new honeycomb.BRITE.Import.GTTSImport(new File(convertFilename));
      im.convert(briteFile);
    }
    else if (extFormat.equals("skitter")) {
    	honeycomb.BRITE.Import.SkitterImport im = new honeycomb.BRITE.Import.SkitterImport(new File(convertFilename), honeycomb.BRITE.Model.ModelConstants.RT_FILE);
      im.convert(briteFile);
    }
    else if (extFormat.equals("inet")) {
    	honeycomb.BRITE.Import.InetImport im = new honeycomb.BRITE.Import.InetImport(new File(convertFilename), honeycomb.BRITE.Model.ModelConstants.RT_FILE);
      im.convert(briteFile);
    }   
    else if (extFormat.equals("scan")) {
    	honeycomb.BRITE.Import.SCANImport im = new honeycomb.BRITE.Import.SCANImport(new File(convertFilename), honeycomb.BRITE.Model.ModelConstants.RT_FILE);
      im.convert(briteFile);
    }   
    else {
