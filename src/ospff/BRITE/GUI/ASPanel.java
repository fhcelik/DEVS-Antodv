...
public final class ASPanel extends JPanel implements ActionListener {
  ...
  public void ConvertFileToBriteFormat() {
    ...
    ospff.BRITE.Util.Util.MSG("Converting AS import file to BRITE..");
    
    String briteFile=file+".brite";
    if (extFormat.equals("nlanr")) {
    	ospff.BRITE.Import.NLANRImport im = new ospff.BRITE.Import.NLANRImport(new File(convertFilename), ospff.BRITE.Model.ModelConstants.AS_FILE);
      im.convert(briteFile);
    }
    else if (extFormat.equals("gtitm")) {
    	ospff.BRITE.Import.GTImport im = new ospff.BRITE.Import.GTImport(new File(convertFilename), ospff.BRITE.Model.ModelConstants.AS_FILE);
      im.convert(briteFile);
    }    
	...
	//      Util.Util.ERR("Transit-Stub can only be imported at router level..");
      ...
      //im.convert(briteFile);
    	...
    	ospff.BRITE.Import.SkitterImport im = new ospff.BRITE.Import.SkitterImport(new File(convertFilename), ospff.BRITE.Model.ModelConstants.AS_FILE);
      im.convert(briteFile);
    }
    else if (extFormat.equals("inet")) {
    	ospff.BRITE.Import.InetImport im = new ospff.BRITE.Import.InetImport(new File(convertFilename), ospff.BRITE.Model.ModelConstants.AS_FILE);
      im.convert(briteFile);
    }   
    else if (extFormat.equals("scan")) {
    	ospff.BRITE.Import.SCANImport im = new ospff.BRITE.Import.SCANImport(new File(convertFilename), ospff.BRITE.Model.ModelConstants.AS_FILE);
      im.convert(briteFile);
    }   
    else {
