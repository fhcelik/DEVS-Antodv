...
import ospff.BRITE.Model.Model;
...
public final class RouterPanel extends JPanel  implements ActionListener {
 ...
 /** converts import format to brite format and returns name of new file. */
  public void ConvertFileToBriteFormat() {
    ...
    ospff.BRITE.Util.Util.MSG("Converting Router-level import file to BRITE..");
    
    String briteFile=file+".brite";
    if (extFormat.equals("nlanr")) {
    	ospff.BRITE.Import.NLANRImport im = new ospff.BRITE.Import.NLANRImport(new File(convertFilename), ospff.BRITE.Model.ModelConstants.RT_FILE);
	im.convert(briteFile);
    }
    else   if (extFormat.equals("gtitm")) {
    	ospff.BRITE.Import.GTImport im = new ospff.BRITE.Import.GTImport(new File(convertFilename), ospff.BRITE.Model.ModelConstants.RT_FILE);
      im.convert(briteFile);
    }    
    else if (extFormat.equals("gtts")) {
    	ospff.BRITE.Import.GTTSImport im = new ospff.BRITE.Import.GTTSImport(new File(convertFilename));
      im.convert(briteFile);
    }
    else if (extFormat.equals("skitter")) {
    	ospff.BRITE.Import.SkitterImport im = new ospff.BRITE.Import.SkitterImport(new File(convertFilename), ospff.BRITE.Model.ModelConstants.RT_FILE);
      im.convert(briteFile);
    }
    else if (extFormat.equals("inet")) {
    	ospff.BRITE.Import.InetImport im = new ospff.BRITE.Import.InetImport(new File(convertFilename), ospff.BRITE.Model.ModelConstants.RT_FILE);
      im.convert(briteFile);
    }   
    else if (extFormat.equals("scan")) {
    	ospff.BRITE.Import.SCANImport im = new ospff.BRITE.Import.SCANImport(new File(convertFilename), ospff.BRITE.Model.ModelConstants.RT_FILE);
      im.convert(briteFile);
    }   
    else {
