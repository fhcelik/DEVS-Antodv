...
import ospff.BRITE.Graph.GraphConstants;
import ospff.BRITE.Graph.Node;
import ospff.BRITE.Model.FileModel;
...
public class NSExport {
	...
	public NSExport(Topology t, File outFile) {
		...
		/*
		 * try { br = new BufferedReader(new FileReader("nsOptions.conf")); }
		 * catch (IOException e) {
		 * Util.ERR("Error creating BufferedReader for file: nsOptions.conf",
		 * e); }
		 */
	}
	...
	public void export() throws Exception {
		...
		bw.write("\t#nodes:");
		bw.newLine();
		bw.write("\tset num_node " + nodes.length);
		bw.newLine();
		bw.write("\tfor {set i 0} {$i < $num_node} {incr i} {");
		bw.newLine();
		bw.write("\t   set n($i) [$ns node]");
		bw.newLine();
		bw.write("\t}");
		bw.newLine();
		bw.newLine();
		...
		for (int i = 0; i < edges.length; ++i) {
			...
			bw.write("$n(" + srcIndex + ") $n(" + dstIndex + ") " + e.getBW()
					+ "Mb " + e.getDelay() + "ms $qtype");
			bw.newLine();
		}
		bw.newLine();
	...
	public static void convert(String briteFile, int format) throws Exception {
		FileModel f = new FileModel(
				ospff.BRITE.Import.ImportConstants.BRITE_FORMAT, briteFile,
				format);
