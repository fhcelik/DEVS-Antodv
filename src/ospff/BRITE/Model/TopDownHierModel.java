...
package ospff.BRITE.Model;

import java.util.ArrayList;
import java.util.Random;

import ospff.BRITE.Graph.ASEdgeConf;
...
import ospff.BRITE.Topology.Topology;
...
public class TopDownHierModel extends Model {
	...
	public TopDownHierModel(ArrayList Models, int edgeConnMethod, int k,
			int bwInter, double interMin, double interMax, int bwIntra,
			double intraMin, double intraMax) {
		if (Models.size() < 2) {
			System.out
					.println("[BRITE ERROR] Top Down Hierarchical Model requires more than one level.");
			System.exit(0);
		}
		this.Models = Models;
	...
	public String toString() {
		String modelParams = "Model (" + ModelConstants.HI_TOPDOWN
				+ " - TopDown)\n";
		modelParams += ((Model) Models.get(0)).toString()
				+ ((Model) Models.get(1)).toString();
		return modelParams;
	}
	...
	/**
	 * Node s and Node d are nodes are AS neighbors. Here, we connect the
	 * corresponding router level graphs of s and d according to edgeConn and
	 * update our flattened topology, gFlat.
	 * 
	 */
	public void ConnectNodes(Node s, Node d, Graph gFlat, int direction) {
		...
		if (edgeConn == ModelConstants.TD_RANDOM) {
			...
			Util.ERR("EdgeConn Method " + edgeConn
					+ " not found/implemented yet.");
		}

		if (sFlat == null || dFlat == null)
			Util
					.ERR("NULL NODE found when connecting topologies in TopDownHierModel.");

		((RouterNodeConf) sFlat.getNodeConf())
				.setType(ModelConstants.RT_BORDER);
		((RouterNodeConf) dFlat.getNodeConf())
				.setType(ModelConstants.RT_BORDER);
		...
		/*
		 * set bandwidth between border nodes according to bwInter and interMin,
		 * interMax
		 */
		AssignInterBW(e);
	}
	...
	private void AssignInterBW(Edge e) {
		...
		if (bwInter == ModelConstants.BW_CONSTANT)
			...
			e.setBW(Distribution.getParetoRandom(BWRandom, interMin, interMax,
					1.2));
	...
	public void ConnectTopologies(Graph g0, Graph gFlat) {
		...
		for (int i = 0; i < size; ++i) {
			...
			ConnectNodes(e.getSrc(), e.getDst(), gFlat, e.getDirection()); /*
																			 * connect
																			 * the
																			 * routers
																			 * of
	...
	public Graph FlattenGraph(Graph g0) {
		/*
		 * initialize ArrayList to numNodes of g0 - since this is number of
		 * graphs at bottom level
		 */
	...
	public Graph Generate() {
		...
		Graph g0 = ((Model) Models.get(0)).Generate(); // g0 is toplevel graph
														// (AS)
		...
		/*
		 * if (Models.get(1) instanceof FileModel) FileGraph =
		 * ((Model)Models.get(1)).generate();
		 */
		Node x=new Node();
		x.nodeCount=-1;
		...
		for (int i = 0; i < vec.length; ++i) {
			...
			if (n.getNodeConf() instanceof ASNodeConf) {
				Util.MSG("Generating Router Topology # " + (i + 1) + " of"
						+ vec.length + "...");
				Topology t = new Topology((Model) Models.get(1)); // level1
																	// graphs

				/*
				 * 1) set this AS node's corresponding router topology to the
				 * one generated by our level one model, and 2) for all the
				 * routers in that topology, assign them asID to be N's id
				 */
		...
		/*
		 * Step 3: Now interconnect the disconnected portions according to
		 * edgeConn
		 */
	 ...
	 * 
	 * RouterWaxman rw = new RouterWaxman(1000, 1000, 1000, 1, 2, (double)0.2,
	 * (double)0.1, 1, 1, 10, 10); ASWaxman aw = new ASWaxman(10, 1000, 1000, 1,
	 * 2, (double)0.2,(double) 0.1, 1, 1, 10, 10); ArrayList models = new
	 * ArrayList(); models.add(aw); models.add(rw);
	 * 
	 * TopDownHierModel td = new TopDownHierModel(models, 1, 2, 1, 10, 100, 2,
	 * 500, 700); td.Generate();
	 * 
	 * }
	 */
