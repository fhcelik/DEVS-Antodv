...
package ospff.BRITE.Main;

import java.io.File;
import java.util.HashSet;

import ospff.BRITE.Export.BriteExport;
import ospff.BRITE.Export.DEVSExport;
import ospff.BRITE.Export.DMLExport;
import ospff.BRITE.Export.JSimExport;
import ospff.BRITE.Export.NSExport;
import ospff.BRITE.Export.OtterExport;
import ospff.BRITE.Graph.Graph;
import ospff.BRITE.Model.Model;
import ospff.BRITE.Topology.Topology;
import ospff.BRITE.Util.RandomGenManager;
...
final public class Brite { 
  ...
  public static Topology getTopology(){
	...
	System.out.println("\tConnected");
	  ...
	  be.export();
	  ospff.BRITE.Visualizer.Visualize.ciz(outFile+".brite");
      }
	 ...
	 Util.MSG("Exporting Topology in DML Format to: " + outFile+".dml");
	 ...
	 de.export();
	  ...
	  Util.MSG("Exporting Topology in NS Format to: " + outFile+".tcl");
	 ...
	 ne.export();
       ...
       if (exportFormats.contains("DEVS-Suite")) {
    		 Util.MSG("Exporting Topology in DEVS-Suite Format to: " + outFile+".java");
    		 DEVSExport ne = new DEVSExport(t, new File("src/ospff/"+outFile+".java"));
    		 ne.export();
    	       }
