...
public class FModelView extends JPanel
{
    ...
    public void loadModel(FModel rootModel)
    {
        ...
        JButton testInput = new JButton("Inject..");
        testInput.addActionListener(listener);
        testInput.setActionCommand("INJECT");	
        JButton trackingInfo = new JButton("Tracking..");
        trackingInfo.setActionCommand("TRACKING");
        trackingInfo.addActionListener(listener);
        
        //If tracking is not selected, tracking option will be disabled
        if(!View.isTracking)
         	trackingInfo.setEnabled(false);
        
        Options = new JPanel(new GridLayout(2,1));      
        
        JPanel inset = new JPanel(new GridLayout(1,2));
        inset.add(testInput);
        inset.add(trackingInfo);      
    ...
    private class UniversalListener implements TreeSelectionListener, ActionListener
    {
        ...
        public void actionPerformed(ActionEvent e) 
        {
            ...
            if (node != null)
            {
                ...
                else if (actionCmd.equalsIgnoreCase("TRACKING"))
                	tracking.showTrackerSettingsDialog((FModel)node.getUserObject());  
            }
        }
    }
