...
public class atomicSimulator implements AtomicSimulatorInterface {// for usual
																	// devs
	...
	public synchronized void initialize() { // for non real time usage, assume
	...
	public synchronized void initialize(double currentTime) { // for real time
																// usage
		myModel.initialize();
		tL = currentTime;
		tN = tL + myModel.ta();
	...
	public synchronized void wrapDeltfunc(double t, MessageInterface x) {
		if (x == null) {
			System.out.println("ERROR RECEIVED NULL INPUT  "
					+ myModel.toString());
			return;
		}
	...
	public void simulate(int numIter) {
		...
		System.out.println("Terminated Normally at ITERATION " + i + " ,time: "
				+ tN);
	}
	...
	public void simInject(double e, MessageInterface m) {
		...
		if (e <= myModel.ta()) {
			...
			System.out.println("Time: " + tL
					+ " ,ERROR input rejected : elapsed time " + e
					+ " is not in bounds.");
	}
	 ...
	 * 
	 * @param waitTime
	 *            The amount of simulation time to wait before injecting the
	 *            input.
	 */
